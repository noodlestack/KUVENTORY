...
- **Resolution**: This will be fixed once the Django PostgreSQL backend is integrated.
...
While the frontend enforces calculations (e.g., preventing negative inventory), a determined user could bypass these client-side restrictions. True data integrity will be enforced by the PostgreSQL database schema and backend serializers.
