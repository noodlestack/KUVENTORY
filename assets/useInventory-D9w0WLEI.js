import{r as e}from"./rolldown-runtime-QTnfLwEv.js";import{i as t}from"./utils-2Y7OFES2.js";import{Bt as n}from"./index-B7UPqq1F.js";var r=e(t(),1),i={getInventory:async()=>{try{let{data:e,error:t}=await n.from(`inventory_balances`).select(`
          id,
          quantity,
        ...
        `);if(t)throw t;return(e||[]).map(e=>{let t=e.quantity===0?`Out of Stock`:e.quantity<=(e.item.minimum_stock_level||0)?`Low Stock`:`In Stock`;return{id:e.item.id,itemCode:e.item.stock_code,name:e.item.name,categoryId:e.item.category?.id||``,categoryName:e.item.category?.name||`Uncategorized`,unit:e.item.unit?.code||`pcs`,supplier:`Default Supplier`,beginningStock:0,addedStock:0,totalStock:e.quantity,morningSales:0,afternoonSales:0,endingStock:e.quantity,cost:e.item.cost_price||0,sellingPrice:e.item.selling_price||0,minStockLevel:e.item.minimum_stock_level||0,storageLocation:e.location?.name||`Main Warehouse`,status:t,lastUpdated:e.item.updated_at||new Date().toISOString(),createdAt:e.item.created_at||new Date().toISOString()}})}catch{return[]}},createItem:async(e,t)=>{let r=null;if(e.unit){let{data:t}=await n.from(`units_of_measure`).select(`id`).eq(`code`,e.unit).limit(1).single();t&&(r=t.id)}let{data:i,error:a}=await n.from(`stock_items`).insert({name:e.name,stock_code:e.itemCode,category_id:e.categoryId,unit_of_measure_id:r,tracking_type:`PORTION`,minimum_stock_level:e.minStockLevel,cost_price:e.cost,selling_price:e.sellingPrice}).select(`*, category:categories(id, name), unit:units_of_measure(code)`).single();if(a)throw a;let{data:o}=await n.from(`inventory_locations`).select(`id`).limit(1).single();return o&&await n.from(`inventory_balances`).insert({stock_item_id:i.id,location_id:o.id,current_quantity:0}),{id:i.id,itemCode:i.stock_code,name:i.name,categoryId:i.category?.id||``,categoryName:i.category?.name||t,unit:i.unit?.code||e.unit,supplier:e.supplier,beginningStock:0,addedStock:0,totalStock:0,morningSales:0,afternoonSales:0,endingStock:0,cost:i.cost_price,sellingPrice:i.selling_price,minStockLevel:i.minimum_stock_level,storageLocation:e.storageLocation,status:`Out of Stock`,notes:e.notes,lastUpdated:new Date().toISOString(),createdAt:new Date().toISOString()}},updateItem:async(e,t,r)=>{let{data:i,error:a}=await n.from(`stock_items`).update({name:t.name,stock_code:t.itemCode,category_id:t.categoryId,minimum_stock_level:t.minStockLevel,cost_price:t.cost,selling_price:t.sellingPrice}).eq(`id`,e).select(`*, category:categories(id, name), unit:units_of_measure(code)`).single();if(a)throw a;let{data:o}=await n.from(`inventory_balances`).select(`current_quantity`).eq(`stock_item_id`,e).limit(1).single(),s=o?Number(o.current_quantity):0,c=s===0?`Out of Stock`:s<=(i.minimum_stock_level||0)?`Low Stock`:`In Stock`;return{id:i.id,itemCode:i.stock_code,name:i.name,categoryId:i.category?.id||``,categoryName:i.category?.name||r,unit:i.unit?.code||t.unit,supplier:t.supplier,beginningStock:0,addedStock:0,totalStock:s,morningSales:0,afternoonSales:0,endingStock:s,cost:i.cost_price,sellingPrice:i.selling_price,minStockLevel:i.minimum_stock_level,storageLocation:t.storageLocation,status:c,notes:t.notes,lastUpdated:new Date().toISOString(),createdAt:i.created_at||new Date().toISOString()}},getMovements:async()=>{try{let{data:e,error:t}=await n.from(`stock_movements`).select(`
          id,
          movement_type,
          quantity,
          reference_id,
